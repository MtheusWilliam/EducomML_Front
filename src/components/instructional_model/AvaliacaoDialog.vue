    ...
    <v-card>
        <v-card-title style="background-color:#63B0B0; color:white;">
            ...
            <v-col cols="7" class="pl-11" style="margin-right: -16px;">
                <v-spacer></v-spacer>
                <v-menu top="top" width="300px" origin="center center" :offset-y="true" transition="scale-transition">
        ...
        <v-card-text>
            <v-form ref="form" v-model="valid" lazy-validation class="mt-3">
                ...
                <v-text-field id="avaliacaoNameArea" v-model="avaliacaoName" :counter="100" :rules="avaliacaoNameRules" required class="mt-2 mb-4"></v-text-field>
                    ...
                    <v-row v-for="(item,i) in questionsControl" :key="i">
                        ...
                        <v-col cols="2">
                            <v-menu top="top" width="300px" origin="center center" :offset-y="true" transition="scale-transition">
                                ...
                                <v-list>
                                    <v-list-item v-for="(itemType, idItemType) in itemsMobileMedia" :key="idItemType" @click="addMobileMediaQuestion(i, idItemType+1)">
                        ...
                        <v-row>
                            <v-col>
                                <v-text-field v-model="questionsControl[i].descriptionQuestion" label="Descrição da questão" required></v-text-field>
                            </v-col>
                            <v-col cols="3" class="mt-3" v-if="item.typeQuestion === 1">
                                <v-btn color="primary" style="color:white;" @click="addAlternative(i)">Adicionar Alternativa</v-btn>
                            </v-col>
                        </v-row>
                        <div v-if="questionsControl[i].mobileMedias.length > 0" style="display: block; width: 100%; margin-top:-20px;">
                            <v-col cols="12">
                                <div v-for="(mobilemedia, idmobile) in questionsControl[i].mobileMedias" :key="idmobile">
                                    <v-row v-if="mobilemedia.type === 1">
                                        <v-spacer></v-spacer>
                                        <v-file-input style="width: 165px;" label="UPLOAD IMAGE" filled @change="atualizaMetaQuestions" v-model="mobilemedia.object" accept="image/*" prepend-icon="mdi-camera"></v-file-input>
                                        <v-btn icon="icon" class="mt-2 ml-3">
                                            <v-icon x-large color="red" @click="deleteMobileMediaQuestion(i, idmobile)">mdi-minus-box</v-icon>
                                        </v-btn>
                                        <v-spacer></v-spacer>
                                    </v-row>
                                    <v-row v-if="mobilemedia.type === 2">
                                        <v-spacer></v-spacer>
                                        <v-file-input style="width: 165px;" label="UPLOAD VIDEO" filled @change="atualizaMetaQuestions" v-model="mobilemedia.object" accept="video/*" prepend-icon="mdi-video"></v-file-input>
                                        <v-btn icon="icon" class="mt-2 ml-3">
                                            <v-icon x-large color="red" @click="deleteMobileMediaQuestion(i, idmobile)">mdi-minus-box</v-icon>
                                        </v-btn>
                                        <v-spacer></v-spacer>
                        ...
                        <div v-if="item.typeQuestion === 1" style="display: block; width: 100%; margin-top:-20px;">
                            <p style="color:#f43e00; margin-bottom: -10px;">
                                <strong>*AVISO:</strong> Marque o checkbox da alternativa caso ela esteja correta.
                            </p>
                            <div style="display: flex; margin-bottom:-30px;" v-for="(itemAlternative,indexAlternative) in questionsControl[i].answersAlternatives" :key="indexAlternative">
                                <v-col cols="1" class="mt-5" style=" margin-right: -40px;font-size:1.3em;">
                                    <strong>{{idObjAnswerItems[indexAlternative]}})</strong>
                                </v-col>
                                <v-col cols="7">
                                    <v-text-field v-model="questionsControl[i].answersAlternatives[indexAlternative].answers" label="Descrição da alternativa" :rules="alternativeDescriptionRules" required></v-text-field>
                                </v-col>
                                <v-col cols="1" style="margin-right:-40px;">
                                    <v-checkbox v-model="questionsControl[i].answersAlternatives[indexAlternative].isTrue"></v-checkbox>
                                </v-col>
                                <v-col cols="1">
                        ...
                        <div v-else-if="item.typeQuestion === 2" style="display: block; width: 100%; margin-top:-20px;">
                            <v-col cols="12">
                                <v-textarea v-model="questionsControl[i].correctAnswer" background-color="#F2F3F3" clearable clear-icon="mdi-close-circle" class="mt-2" name="input-3-1" label="Resposta da questão" filled auto-grow required></v-textarea>
                            </v-col>
                        </div>
                    </v-row>
                </v-container>
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-menu v-if="questionsControl.length > 0" top="top" width="300px" origin="center center" :offset-y="true" transition="scale-transition">
                <template v-slot:activator="{ on }">
    ...
    <div class="text-center">
        <v-dialog v-model="dialog_alert" width="500">
            <v-card>
                ...
                <v-card-text class="mt-3" style="font-size: 1.3em;">Para criar uma avaliação, é necessário criar pelo uma questão.</v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        <v-dialog v-model="dialog_alert2" width="500">
            <v-card>
                ...
                <v-card-text class="mt-3" style="font-size: 1.3em;">
                    ...
                    <br /> Ajuste a questão {{questionAjust}}.
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        async getAvaliacao() {
                        ...
                        await this.instructionalelement.questions.forEach(async function(
                            ...
                            if (
                                ...
                                await vm.questionsControl.push({
                                    ...
                                    descriptionQuestion: elementQuestion.descriptionquestion,
                                    correctAnswer: elementQuestion.resolutionquestion[0].correctanswer,
                                    mobileMedias: [],
                                    url: elementQuestion.url,
            ...
            async postQuestions() {
                var instructionalelement = {
                    ...
                    fk_instructionalelementtype: `http://127.0.0.1:8000/instrucelementtype/` +
                                    ...
                                    await axios
                                        .put(
                                            elementQuestion.url, {
                                                ...
                                                descriptionquestion: elementQuestion.descriptionQuestion,
                                                typequestion: `http://127.0.0.1:8000/questiontype/` +
                                                    elementQuestion.typeQuestion +
                                                    `/`,
                        ...
                        .then(async function(resposta) {
                            ...
                            await vm.questionsControl.forEach(async function(
                                ...
                                await axios
                                    ...
                                    .then(async function(resposta2) {
                                        /* INSERÇÃO DOS MOBILEMEDIAS DAS QUESTÕES DA AVALIAÇÃO/EXERCICIO */
                                        await elementQuestion.mobileMedias.forEach(async function(
                                            elementMobile,
                                            indexMobile
                                        ) {
                        ...
                        for (var i = 0; i < this.questionsControl.length; i++) {
                            if (aux === 1) {
                                if (this.questionsControl[i].typeQuestion === 1) {
                                    ...
                                    for (
                                        var j = 0; j < this.questionsControl[i].answersAlternatives.length; j++
                                    ) {
