            ...
            <v-expansion-panel v-for="(item,i) in 1" :key="i">
                <v-expansion-panel-header color="purple" style="color:white;">
                    <v-row>
                        <v-col cols="9">
                            <p>[Dominio]
                                {{ dominio_data.nameknowledgedomain }}</p>
                        </v-col>
                        <v-col cols="3">
                            <v-dialog v-model="dialog_modulo" persistent="persistent" max-width="600px">
                                ...
                                <ModuloDialog
                                    @dialog_modulo="dialog_modulo = !dialog_modulo"
                                    idDomain="dominio.idknowledgedomain"/>
    ...
    export default {
        ...
        data: () => ({
            ...
            identifierNameRules: [
                ...
                v => (v && v.length <= 25) || "Nome do identificador deve ter no máximo 25 caracteres"
            ],
            moduloTitle: "",
            moduloTitleRules: [
                v => !!v || "É necessário descrever o título do módulo",
                v => (v && v.length <= 40) || "O título do módulo deve ter no máximo 40 caracteres"
            ],
            moduloSubtitle: "",
            moduloSubtitleRules: [
                v => !!v || "É necessário descrever o subtítulo do módulo",
                v => (v && v.length <= 40) || "O subtítulo do módulo deve ter no máximo 40 caracteres"
            ],
            select: null,
            newItems: [
                "Módulo", "SubMódulo"
 ...
 */
        ...
        methods: {
            putDominio() {
                ...
                axios
                    .put(
                        "http://127.0.0.1:8000/knowledgedomain/" + this.dominio.idknowledgedomain +
                                "/",
                        {
            ...
            validate() {
                if (this.domainName && this.domainContentTitle && this.domainAuthorsName) {
                    this.putDominio();
