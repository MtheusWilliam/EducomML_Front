              ...
              >
                ...
                <v-col cols="2">
                  <v-text-field
                    ...
                    type="number"
                    :suffix="
                      assessmentControl[i].typeThreshold === 1 ? '%' : ''
                    "
                    required
                  ></v-text-field>
                </v-col>
          ...
          >
            ...
            <v-col cols="3">
              <v-text-field
                ...
                :rules="[
                  (v) =>
                    !!v || 'Necessário informar identificador da prioridade',
                ]"
  ...
  export default {
    ...
    methods: {
      ...
      async postAssessment() {
        ...
        await this.assessmentControl.forEach(async (assessment) => {
          ...
          if (assessment.url) {
                ...
                if (assessment.valueType === "Single") {
                  if (assessment.ranges) {
                    assessment.ranges.forEach((range) => {
                      if (range.url) {
                        vm.Api().delete(range.url, header);
                      }
                    });
                  }
                  await vm.Api().put(
                  ...
                  if (assessment.single.url) {
                    vm.Api().delete(assessment.single.url, header);
                  }
            ...
            await this.Api()
              .post("/assessmentparameter/", auxAssessment, header)
              .then(async function(resposta) {
                if (assessment.valueType === "Single") {
                  await vm.Api().post(
                    "/single/",
      ...
      async deletaSingle(urlSingle) {
        var header = await this.$store.dispatch("getHeader");
        await this.Api().delete(urlSingle, header);
      },
      addPriorKnowledge() {
        this.priorControl.push({
          namepriorknowledge: "",
      ...
      async deletaPriorKnowledge(idPrior) {
        if (this.priorControl[idPrior].url) {
          var header = await this.$store.dispatch("getHeader");
          await this.Api().delete(this.priorControl[idPrior].url, header);
        }
        if (idPrior == 0) {
